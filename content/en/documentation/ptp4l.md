...
* Tables for configuring unicast discovery begin with `%[unicast_master_table]`. See [UNICAST DISCOVERY OPTIONS](#unicast-discovery-options), below.
...
: Each table entry specifies the transport type and network address of a potential remote server.  If multiple servers are specified, then unicast negotiation will be performed with each if them.
